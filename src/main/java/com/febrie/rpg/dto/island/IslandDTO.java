 ...
 */
public record IslandDTO(@NotNull IslandCoreDTO core,                // 기본 정보
     ...
     * 신규 섬 생성용 기본 생성자
     */
    public static @NotNull IslandDTO createNew(String islandId, String ownerUuid, String ownerName, String islandName) {
    ...
    public boolean isMember(String playerUuid) {
        ...
        return membership.members().stream().anyMatch(member -> member.uuid().equals(playerUuid));
    }
    ...
    public Map<String, Object> toMap() {
        ...
        // Core 정보
        Map<String, Object> map = new HashMap<>(core.toMap());

        // Membership 정보
        List<Map<String, Object>> membersList = membership.members().stream().map(IslandMemberDTO::toMap)
                .collect(Collectors.toList());
        map.put("members", membersList);

        List<Map<String, Object>> workersList = membership.workers().stream().map(IslandWorkerDTO::toMap)
                .collect(Collectors.toList());
        ...
        // Social 정보
        List<Map<String, Object>> pendingInvitesList = social.pendingInvites().stream().map(IslandInviteDTO::toMap)
                .collect(Collectors.toList());
        map.put("pendingInvites", pendingInvitesList);

        List<Map<String, Object>> recentVisitsList = social.recentVisits().stream().map(IslandVisitDTO::toMap)
                .collect(Collectors.toList());
     ...
     */
    @Contract("_ -> new")
    public static @NotNull IslandDTO fromMap(@NotNull Map<String, Object> map) {
        // Core DTO 생성
        ...
        IslandMembershipDTO membership = new IslandMembershipDTO(core.islandId(), members, workers, contributions);
        ...
        IslandSocialDTO social = new IslandSocialDTO(core.islandId(), pendingInvites, recentVisits);
        ...
        IslandConfigurationDTO configuration = new IslandConfigurationDTO(core.islandId(), spawnData, upgradeData, permissions, settings);

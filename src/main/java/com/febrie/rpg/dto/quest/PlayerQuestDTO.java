...
public record PlayerQuestDTO(String playerId, Map<String, QuestProgress> activeQuests,
     ...
     * 방어적 복사를 위한 생성자
     */
    public PlayerQuestDTO(String playerId, Map<String, QuestProgress> activeQuests, Map<String, CompletedQuestDTO> completedQuests, long lastUpdated) {
    ...
    public JsonObject toJsonObject() {
        ...
        // activeQuests
        fields.add("activeQuests", JsonUtil.createMapField(activeQuests, progress -> JsonUtil.createMapValue(progress.toJsonObject())));

        // completedQuests
        fields.add("completedQuests", JsonUtil.createMapField(completedQuests, completed -> JsonUtil.createMapValue(completed.toJsonObject())));
    ...
    public static PlayerQuestDTO fromJsonObject(@NotNull JsonObject json) {
        ...
        // activeQuests
        Map<String, QuestProgress> activeQuests = JsonUtil.getMapField(fields, "activeQuests", key -> key, obj -> {
            JsonObject mapValue = obj.getAsJsonObject("mapValue");
            return QuestProgress.fromJsonObject(mapValue);
        });

        // completedQuests
        Map<String, CompletedQuestDTO> completedQuests = JsonUtil.getMapField(fields, "completedQuests", key -> key, obj -> {
            JsonObject mapValue = obj.getAsJsonObject("mapValue");

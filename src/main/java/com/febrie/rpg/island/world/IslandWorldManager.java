...
public class IslandWorldManager {
    ...
    private void generateIslandWithFAWE(int centerX, int centerZ, int size, String biomeName, String template) {
        ...
        TaskManager.taskManager().async(() -> {
            ...
            try (EditSession editSession = WorldEdit.getInstance()
                ...
                LogUtil.error("섬 생성 중 오류 발생: " + e.getMessage());
                e.printStackTrace();
            }
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            TaskManager.taskManager().async(() -> {
                ...
                try (EditSession editSession = WorldEdit.getInstance()
                    ...
                    LogUtil.error("섬 확장 중 오류: " + e.getMessage());
                    e.printStackTrace();
                }
            });
    ...
    private void clearIsland(int centerX, int centerZ, int size) {
        ...
        TaskManager.taskManager().async(() -> {
            ...
            try (EditSession editSession = WorldEdit.getInstance()
                ...
                LogUtil.error("섬 제거 중 오류: " + e.getMessage());
                e.printStackTrace();
            }
        });
    }
    ...
    private static class EmptyChunkGenerator extends ChunkGenerator {
        ...
        public void generateNoise(@NotNull WorldInfo worldInfo, @NotNull Random random, 
                                  int chunkX, int chunkZ, @NotNull ChunkGenerator.ChunkData chunkData) {
            // 빈 청크 생성 (아무것도 하지 않음)
        }
    }
}
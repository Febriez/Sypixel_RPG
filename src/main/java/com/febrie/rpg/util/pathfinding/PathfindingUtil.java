...
public class PathfindingUtil {
    ...
    public static List<Location> findPath(@NotNull Location start, @NotNull Location target) {
        ...
        while (!openSet.isEmpty() && iterations < MAX_ITERATIONS) {
            ...
            // F 비용이 가장 낮은 노드 선택
            PathNode currentNode = openSet.getFirst();
    ...
    private static boolean isWalkable(@NotNull PathNode node, @NotNull World world) {
        ...
        return isPassable(currentBlock) && isPassable(headBlock) && isSolid(footBlock);
    }
    ...
    private static boolean isPassable(@NotNull Block block) {
        ...
        return material.isAir() || material == Material.WATER || material == Material.LAVA || material == Material.GRASS_BLOCK || material == Material.TALL_GRASS || material == Material.FERN || material == Material.LARGE_FERN || material.name().contains("SAPLING") || material.name().contains("FLOWER") || material == Material.SNOW || material == Material.VINE || material == Material.LADDER || material.name().contains("TORCH") || material.name().contains("SIGN") || material.name().contains("PRESSURE_PLATE") || material.name().contains("BUTTON");
    }
    ...
    private static boolean isSolid(@NotNull Block block) {
        ...
        // 대부분의 블록은 고체로 간주
        return material.isSolid() || material.name().contains("SLAB") || material.name().contains("STAIRS") || material == Material.SNOW_BLOCK || material == Material.ICE || material == Material.PACKED_ICE;
    }
    ...
    public static boolean isPathValid(@NotNull List<Location> path) {
        ...
        World world = path.getFirst().getWorld();

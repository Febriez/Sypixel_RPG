...
public class AdminCommandHandler implements CommandExecutor, TabCompleter {
    ...
    private void registerSubCommands() {
        ...
        // registerSubCommand(new GiveItemCommand(plugin)); // TODO: 구현 필요
        registerSubCommand(new ReloadCommand(plugin));
        registerSubCommand(new SaveCommand(plugin));
        // registerSubCommand(new QuestCommand(plugin)); // TODO: 구현 필요
        // registerSubCommand(new IslandAdminCommand(plugin)); // TODO: 구현 필요
        // registerSubCommand(new PlayerInfoCommand(plugin)); // TODO: 구현 필요
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        if (!sender.hasPermission("rpg.admin")) {
            sender.sendMessage(ColorUtil.colorize("&c이 명령어를 사용할 권한이 없습니다"));
            return true;
        }
        ...
        if (subCommand == null) {
            sender.sendMessage(ColorUtil.colorize("&c알 수 없는 명령어입니다: " + args[0]));
            sender.sendMessage(ColorUtil.colorize("&7/rpgadmin help - 도움말 보기"));
            return true;
        }
        ...
        if (!sender.hasPermission(subCommand.getPermission())) {
            sender.sendMessage(ColorUtil.colorize("&c이 서브커맨드를 사용할 권한이 없습니다"));
            return true;
        }
        ...
        if (subArgs.length < subCommand.getMinArgs()) {
            sender.sendMessage(ColorUtil.colorize("&c인자가 부족합니다"));
            sender.sendMessage(ColorUtil.colorize("&7사용법: " + subCommand.getUsage()));
            return true;
        }
        
        if (subCommand.getMaxArgs() != -1 && subArgs.length > subCommand.getMaxArgs()) {
            sender.sendMessage(ColorUtil.colorize("&c인자가 너무 많습니다"));
            sender.sendMessage(ColorUtil.colorize("&7사용법: " + subCommand.getUsage()));
            return true;
        }
        ...
        try {
            ...
            sender.sendMessage(ColorUtil.colorize("&c명령어 실행 중 오류가 발생했습니다"));
    ...
    private void showHelp(@NotNull CommandSender sender, @NotNull String pageStr) {
        ...
        sender.sendMessage(ColorUtil.colorize("&6=== RPG Admin Commands (페이지 " + page + "/" + totalPages + ") ==="));
        ...
        if (totalPages > 1) {
            sender.sendMessage(ColorUtil.colorize("&7/rpgadmin help " + (page + 1) + " - 다음 페이지"));
        }
    }
}